 ...
 */
public abstract class Contrail {
     ...
     */
    protected DiffuseParameters params;
     ...
     */
    public Contrail(DiffuseParameters params, Random randomProvider) {
     ...
     * @param params The parameters to use in the model
     */
    public Contrail(DiffuseParameters params) {
        // Initialize variables
        this.rand = new Random();
        this.params = params;
        initialize();
    ...
    /**
     * Helper function which pre-computes values that are used often during the integration of single photons. This
     * saves resources and reduces execution time.
     */
    protected void preComputeHelperVariables() {
        // Iextinction helpers
        this.detsigma = params.getSigmaH() * params.getSigmaV() - Math.pow(params.getSigmaS(), 2);
        this.hDetsigma = 1.0 / (2.0 * detsigma);
        this.c1 = -1. * ((double) params.getAbsorptionFactor() + params.getScatteringFactor()) *
                params.getDropletRadius() * params.getDropletRadius() * params.getNumIceParticles() /
                (params.getDistance() * 2 * Math.sqrt(detsigma));

        this.sqrtPi = Math.sqrt(Math.PI);
    }

    /**
     * Polymorphic method to allow subclasses to initialize physical parameters before initializing scPhFun.
     */
    protected abstract void initialize();

    /**
     * Initializes physical parameters of the contrail. Designed to be called by subclasses in
     * {@link Contrail#initialize()}.
     *
     * @param phyParams A concrete instance of {@link PhysicalParameters} used to calculate the parameters
     */
    protected void initializePhysicalParameters(PhysicalParameters phyParams) {
        if (params.getSpectralBandIndex() == null)
            throw new IllegalArgumentException("Parameters should contain spectral_band_index");
        if (params.getDropletRadius() == null)
            throw new IllegalArgumentException("Parameters should contain radius_droplet");

        double lambda = phyParams.getLambda(params.getSpectralBandIndex());

        double w = 2.0 * params.getDropletRadius() * 1e6;
        double qAbs = phyParams.calcQabs(w, params.getSpectralBandIndex());
        double qSca = phyParams.calcQsca(w, params.getSpectralBandIndex());
        double g = phyParams.calcG(w, params.getSpectralBandIndex());

        params.setLambda(lambda);
        params.setG(g);
        params.setAbsorptionFactor(qAbs);
        params.setScatteringFactor(qSca);
    }

    /**
     * Creates the scattering phase function object with values calculated from the numSca parameter. Call only in
     * constructor and after initializing {@link Contrail#params}!
     */
    ...
    /**
     * Computes the probability of interaction with the contrail for a single photon, according to formula {@code 3.56}
     * in <a href="http://dx.doi.org/10.13140/RG.2.2.34522.75206">Optical Properties of Condensation Trails (Paper)</a>,
     * section {@code 3.5}.
     *
     * @param y0 Starting position with respect to the horizontal axis
     * @param z0 Starting position with respect to the vertical axis
     * @param theta Angle to the vertical axis (for polar coordinate conversion)
     * @param phi Angle to the x-axis (for polar coordinate conversion)
     * @param s Distance s through the contrail
     * @return
     */
    protected double Iextinction(double y0, double z0, double theta, double phi, double s) {
        // a = a' / 2*det(sigma^)
        double a = hDetsigma * (params.getSigmaV() * Math.pow(Math.sin(theta), 2) * Math.pow(Math.sin(phi), 2)
                - 2 * params.getSigmaS() * Math.sin(theta) * Math.cos(theta) * Math.sin(phi)
                + params.getSigmaH() * Math.pow(Math.cos(theta), 2));
        // b = -b' / 2*det(sigma^)
        double b = -hDetsigma * (2 * (params.getSigmaV() * y0 - params.getSigmaS() * z0) * Math.sin(theta) * Math.sin(phi)
                + 2 * (params.getSigmaH() * z0 - params.getSigmaS() * y0) * Math.cos(theta));
        // c = -c' / 2*det(sigma^)
        double c = -hDetsigma * (params.getSigmaV() * y0 * y0 - 2 * params.getSigmaS() * y0 * z0 + params.getSigmaH() * z0 * z0);

        double wa = Math.sqrt(a);
        double f0 = sqrtPi / (2. * wa) * Math.exp(b * b / (4 * a) + c);
        double f1 = f0 * ErrorFunction.erf(wa * s - b / (2 * wa));
        double f2 = f0 * ErrorFunction.erf(-b / (2 * wa));

        return 1.0 - Math.exp(c1 * (f1 - f2));
    }
    ...
    /**
     * Gets the diffuse parameters of the contrail.
     *
     * @return The diffuse parameters of this object
     */
    public DiffuseParameters getParams() {
        return params;
    }

    /**
     * Gets the status flag for multi-threaded operation mode.
     *
     * @return {@code true} if multithreaded mode is enabled, {@code false} otherwise
     */
    public boolean inMultiThreadedMode() {
        return multiThreadedMode;
    }

    /**
     * Sets the status flag for multi-threaded operation mode.
     *
     * @param multiThreadedMode The value to set
     */
    public void setMultiThreadedMode(boolean multiThreadedMode) {
